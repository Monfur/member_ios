...
- (void)viewDidLoad{
  ...
  [self.tabBarController setTabbarHidden:YES animate:NO];
}
...
- (UIButton *)flashButton{
  if (!_flashButton) {
    ...
    UIImage *image = [[MABundleManager manager].defaultBundle
                       getImageFromBundle:@"camera_flashlight_open@2x.png"];
        ...
        [self.cameraManager switchFlashMode:^(AVCaptureFlashMode flashMode) {
          ...
          switch (flashMode) {
            case AVCaptureFlashModeAuto:
              imageName = @"camera_flashlight_auto@2x.png";
              break;
            case AVCaptureFlashModeOff:
              imageName = @"camera_flashlight_disable@2x.png";
              break;
            case AVCaptureFlashModeOn:
              imageName = @"camera_flashlight_open@2x.png";
              break;
            default:
              break;
          }
          UIImage *place = [[MABundleManager manager].defaultBundle
                   getImageFromBundle:imageName];
...
- (UIButton *)switchButton{
  if (!_switchButton) {
    UIButton *button = [UIButton buttonWithType:UIButtonTypeCustom];
    UIImage *image = [[MABundleManager manager].defaultBundle
                        getImageFromBundle:@"camera_overturn@2x.png"];
    [button setBackgroundImage:image
                      forState:UIControlStateNormal];
    [button addActionBlock:^(id sender) {
...
- (UIButton *)snapButton{
  if (!_snapButton) {
    UIButton *button = [UIButton buttonWithType:UIButtonTypeCustom];
    NSBundle *bundle = [MABundleManager manager].defaultBundle;
    UIImage *image = [[bundle getImageFromBundle:@"camera_snap_start.png"] imageWithRenderingMode:UIImageRenderingModeAlwaysTemplate];
...
- (UIButton *)cancelButton{
  if (!_cancelButton) {
    ...
    button.titleLabel.font = [UIFont helveticaFontOfSize:18.0f];
...
- (UIImageView *)snapDecoratedOuter{
  if (!_snapDecoratedOuter) {
    ...
    UIImage *outerImage = [[MABundleManager manager].defaultBundle getImageFromBundle:@"camera_snap_normal_outer.png"];
